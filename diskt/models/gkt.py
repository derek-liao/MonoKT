...
class GKT(nn.Module):
    ...
    Args:
        num_skills (int): total num of unique questions
        hidden_dim (int): hidden dimension for MLP
        embedding_size (int): embedding dimension for question embedding layer
    ...
    """
    def __init__(self, device, num_skills, graph, hidden_dim, embedding_size, graph_type="dense", dropout=0.5):
        ...
        self.graph = nn.Parameter(graph)  # [num_skills, num_skills]
        self.graph.requires_grad = False  # fix parameter


        # one-hot feature and question
        one_hot_feat = torch.eye(self.res_len * self.num_skills).to(device)
        self.one_hot_feat = one_hot_feat
        # self.one_hot_q = torch.eye(self.num_skills, device=self.one_hot_feat.device)
        # zero_padding = torch.zeros(1, self.num_skills, device=self.one_hot_feat.device)
        self.one_hot_q = torch.eye(self.num_skills).to(device)
        zero_padding = torch.zeros(1, self.num_skills).to(device)
        self.one_hot_q = torch.cat((self.one_hot_q, zero_padding), dim=0)
        
        # concept and concept & response embeddings
        self.interaction_emb = nn.Embedding(self.res_len * num_skills, embedding_size)
        # last embedding is used for padding, so dim + 1
        self.emb_c = nn.Embedding(num_skills + 1, embedding_size, padding_idx=-1)

        # f_self function
        mlp_input_dim = hidden_dim + embedding_size
        self.f_self = MLP(mlp_input_dim, hidden_dim, hidden_dim, dropout=dropout, bias=True)

        # f_neighbor functions
        self.f_neighbor_list = nn.ModuleList()

        # f_in functions
        self.f_neighbor_list.append(MLP(2 * mlp_input_dim, hidden_dim, hidden_dim, dropout=dropout, bias=True))
        # f_out functions
        self.f_neighbor_list.append(MLP(2 * mlp_input_dim, hidden_dim, hidden_dim, dropout=dropout, bias=True))


        # Erase & Add Gate
        self.erase_add_gate = EraseAddGate(hidden_dim, num_skills)
        # Gate Recurrent Unit
        self.gru = nn.GRUCell(hidden_dim, hidden_dim, bias=True)
        # prediction layer
        self.predict = nn.Linear(hidden_dim, 1, bias=True)
    ...
    def _aggregate(self, xt, qt, ht, batch_size):
        ...
        Shape:
            xt: [batch_size]
            qt: [batch_size]
            ht: [batch_size, num_skills, hidden_dim]
            tmp_ht: [batch_size, num_skills, hidden_dim + embedding_size]
        Return:
        ...
        x_idx_mat = torch.arange(self.res_len * self.num_skills, device=device)
        x_embedding = self.interaction_emb(x_idx_mat)  # [res_len * num_skills, embedding_size]#the emb for each concept with answer?
        # print(xt[qt_mask])
        # print(self.one_hot_feat)
        masked_feat = F.embedding(xt[qt_mask.to(device)], self.one_hot_feat.to(device))  # [mask_num, res_len * num_skills] A simple lookup table that looks up embeddings in a fixed dictionary and size.
        ...
        concept_idx_mat = self.num_skills * torch.ones((batch_size, self.num_skills), device=device).long()
        concept_idx_mat[qt_mask, :] = torch.arange(self.num_skills, device=device)
        concept_embedding = self.emb_c(concept_idx_mat)  # [batch_size, num_skills, embedding_size]
        ...
        tmp_ht = torch.cat((ht, concept_embedding), dim=-1)  # [batch_size, num_skills, hidden_dim + embedding_size]
        return tmp_ht
    ...
    def _agg_neighbors(self, tmp_ht, qt):
        ...
        Shape:
            tmp_ht: [batch_size, num_skills, hidden_dim + embedding_size]
            qt: [batch_size]
            m_next: [batch_size, num_skills, hidden_dim]
        Return:
        ...
        masked_tmp_ht = tmp_ht[qt_mask]  # [mask_num, num_skills, hidden_dim + embedding_size]
        mask_num = masked_tmp_ht.shape[0]
        self_index_tuple = (torch.arange(mask_num, device=qt.device), masked_qt.long())
        self_ht = masked_tmp_ht[self_index_tuple]  # [mask_num, hidden_dim + embedding_size]
        self_features = self.f_self(self_ht)  # [mask_num, hidden_dim]
        expanded_self_ht = self_ht.unsqueeze(dim=1).repeat(1, self.num_skills, 1)  #[mask_num, num_skills, hidden_dim + embedding_size]
        neigh_ht = torch.cat((expanded_self_ht, masked_tmp_ht), dim=-1)  #[mask_num, num_skills, 2 * (hidden_dim + embedding_size)]
        concept_embedding, rec_embedding, z_prob = None, None, None

     
        adj = self.graph[masked_qt.long(), :].unsqueeze(dim=-1)  # [mask_num, num_skills, 1]
        reverse_adj = self.graph[:, masked_qt.long()].transpose(0, 1).unsqueeze(dim=-1)  # [mask_num, num_skills, 1]
        # self.f_neighbor_list[0](neigh_ht) shape: [mask_num, num_skills, hidden_dim]
        neigh_features = adj * self.f_neighbor_list[0](neigh_ht) + reverse_adj * self.f_neighbor_list[1](neigh_ht)

        # neigh_features: [mask_num, num_skills, hidden_dim]
    ...
    def _update(self, tmp_ht, ht, qt):
        ...
        Shape:
            ...
            ht: [batch_size, num_skills, hidden_dim]
            qt: [batch_size]
            h_next: [batch_size, num_skills, hidden_dim]
        Return:
        ...
        # GNN Aggregation
        m_next, concept_embedding, rec_embedding, z_prob = self._agg_neighbors(tmp_ht, qt)  # [batch_size, num_skills, hidden_dim]
        # Erase & Add Gate
        m_next[qt_mask] = self.erase_add_gate(m_next[qt_mask])  # [mask_num, num_skills, hidden_dim]
        # GRU
        h_next = m_next
        res = self.gru(m_next[qt_mask].reshape(-1, self.hidden_dim), ht[qt_mask].reshape(-1, self.hidden_dim))  # [mask_num * num_skills, hidden_num]
        index_tuple = (torch.arange(mask_num, device=qt_mask.device), )
        h_next[qt_mask] = h_next[qt_mask].index_put(index_tuple, res.reshape(-1, self.num_skills, self.hidden_dim))
    ...
    def _predict(self, h_next, qt):
        ...
        Shape:
            h_next: [batch_size, num_skills, hidden_dim]
            qt: [batch_size]
            y: [batch_size, num_skills]
        Return:
            y: predicted correct probability of all concepts at the next timestamp
        """
        qt_mask = torch.ne(qt, -1)  # [batch_size], qt != -1
        y = self.predict(h_next).squeeze(dim=-1)  # [batch_size, num_skills]
        y[qt_mask] = torch.sigmoid(y[qt_mask])  # [batch_size, num_skills]
        return y
    ...
    def _get_next_pred(self, yt, q_next):
        ...
        """
        next_qt = q_next
        next_qt = torch.where(next_qt != -1, next_qt, self.num_skills * torch.ones_like(next_qt, device=yt.device))
        one_hot_qt = F.embedding(next_qt.long(), self.one_hot_q.to(yt.device))  # [batch_size, num_skills]
    ...
    def forward(self, feed_dict):
        ...
        masked_r = r * (r > -1).long()
        features = q*2 + masked_r
        questions = q
        
        batch_size, seq_len = features.shape
        device = q.device
        ht = Variable(torch.zeros((batch_size, self.num_skills, self.hidden_dim), device=device))
        ...
        for i in range(seq_len):
            ...
            tmp_ht = self._aggregate(xt, qt, ht, batch_size)  # [batch_size, num_skills, hidden_dim + embedding_size]
            h_next, concept_embedding, rec_embedding, z_prob = self._update(tmp_ht, ht, qt)  # [batch_size, num_skills, hidden_dim]
            ht[qt_mask] = h_next[qt_mask]  # update new ht
            yt = self._predict(h_next, qt)  # [batch_size, num_skills]
            if i < seq_len - 1:
...
class EraseAddGate(nn.Module):
    ...
    def __init__(self, feature_dim, num_skills, bias=True):
        super(EraseAddGate, self).__init__()
        # weight
        self.weight = nn.Parameter(torch.rand(num_skills))
    ...
    def forward(self, x):
        ...
        Shape:
            x: [batch_size, num_skills, feature_dim]
            res: [batch_size, num_skills, feature_dim]
        ...
        """
        erase_gate = torch.sigmoid(self.erase(x))  # [batch_size, num_skills, feature_dim]
        # self.weight.unsqueeze(dim=1) shape: [num_skills, 1]
        tmp_x = x - self.weight.unsqueeze(dim=1) * erase_gate * x
        add_feat = torch.tanh(self.add(x))  # [batch_size, num_skills, feature_dim]
