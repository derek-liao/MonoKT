...
class ATKT(nn.Module):
    def __init__(self, num_skills, skill_dim, answer_dim, hidden_dim, attention_dim=80, epsilon=10, beta=0.2, dropout=0.2):
    ...
    def forward(self, feed_dict, perturbation=None):
        ...
        out_dict = {
            "pred": (res * one_hot(c[:, 1:].long(), self.num_skills)).sum(-1),
            "true": r[:, 1:].float(),
